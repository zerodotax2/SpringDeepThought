...
function initComments() {
    ...
    postButton.addEventListener('click', function (e) {
        ...
        if(value.length < 5 || value.length > 1000){
            modal.error('Длина комментария не может быть меньше 5 и не должна превышать 1000 символов');
            return;
        }
        xhr.request({
            method: 'POST',
...
function initAnswer() {
    let answerContent = document.getElementsByClassName("answer-content")[0],
        answerHeader = document.getElementsByClassName("answer-header")[0],
        isAnswerOpen = false;
    ...
    function closeAnswer() {
        ...
        let timer = setInterval(function () {
            count -= 0.05;
            if (count >= -0.1){
                answerContent.style.opacity = count;
            }else{
                clearInterval(timer);
            }
...
function initDoBetter(){
    const btn = document.querySelector('.do-better');
    btn.addEventListener('click', function (e) {
        modal.feedback('Как улучшить?', modal.loadColor, '/services/problems/feedback', problemId);
    });
}
initComments();
initAnswer();

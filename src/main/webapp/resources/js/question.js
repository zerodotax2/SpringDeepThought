...
const userId = document.getElementById('user_id').getAttribute('content'),
        ...
        answerContainer = answersDIV.querySelector('.answer-container');
            ...
            if(response){
                ...
                if(responseAnswer !== undefined && responseAnswer !== ''){
                    ...
                    div.innerHTML = '<div class="answer">' +
                        ...
                        '                            <div class="content">' +
                        '                                <div class="left-side">' +
                        '                                    <svg aria-hidden="true" data-prefix="far" data-icon="check-circle" class="svg-inline--fa fa-check-circle fa-w-16 is-right" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="#4CAF50" d="M256 8C119.033 8 8 119.033 8 256s111.033 248 248 248 248-111.033 248-248S392.967 8 256 8zm0 48c110.532 0 200 89.451 200 200 0 110.532-89.451 200-200 200-110.532 0-200-89.451-200-200 0-110.532 89.451-200 200-200m140.204 130.267l-22.536-22.718c-4.667-4.705-12.265-4.736-16.97-.068L215.346 303.697l-59.792-60.277c-4.667-4.705-12.265-4.736-16.97-.069l-22.719 22.536c-4.705 4.667-4.736 12.265-.068 16.971l90.781 91.516c4.667 4.705 12.265 4.736 16.97.068l172.589-171.204c4.704-4.668 4.734-12.266.067-16.971z"></path></svg>' +
                        ...
                        '                                    <img src="'+appConf.fileServerLocation+responseAnswer.user.userImage+'"/>' +
                        '                                    <div class="info">' +
...
function initAnswers(){
    ...
    Array.forEach(answerRates, function (answerRate) {
        answerRate.addEventListener('click', upAnswerRate);
    });
    function upAnswerRate(e) {
        ...
        xhr.request({
            method: 'POST',
            path: '/services/questions/answers/rating',
            content: JSON.stringify({
                id: answerId,
                rate: rate
            })
    ...
    Array.forEach(answers, function (answer) {
        if(answer.getAttribute('user') === userId){
            userEdit(answer);
        }
    });
}
    ...
    function userEdit(answer){
        ...
        remove.addEventListener('click', function (e) {
           ...
           xhr.request({
               path: '/services/questions/answers',
               method: 'DELETE',
               content: JSON.stringify({
               ...
               if(response){
                  const deleteDTO = JSON.parse(response),
                      answer = document.getElementById('answer-'+deleteDTO.id);
    ...
    function changeAnswer(e){
        ...
        xhr.request({
           ...
           if(response){
               const changeDTO = JSON.parse(response);
               answerEdit.answerContent.innerHTML = changeDTO.htmlContent;
               ...
               answersDIV.insertBefore(editor.editWrapper, answerContainer.lastElementChild);
           } else if(error){
    ...
    function initRightAnswers(){
        ...
        Array.forEach(rightAnswer.isRights, function (elem) {
            if(elem.getAttribute('content') === rightAnswer.rightId){
                elem.classList.add('right');
                rightAnswer.right = elem;
            }else{
                elem.classList.add('noright');
            }
            if(rightAnswer.isOwner){
                elem.classList.add('owner');

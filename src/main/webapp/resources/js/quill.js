              ...
              if (fileInput.files != null && fileInput.files[0] != null) {
                xhr.file('/upload', fileInput.files[0], function (response, error) {
                    if(response){
                        ...
                        _this3.quill.updateContents(new _quillDelta2.default().retain(range.index).delete(range.length).insert({ image: result.fileServerLocation + result.path }), _emitter2.default.sources.USER);
...
function initQuillEditor(editorElem) {

    const qlToolbar = editorElem.querySelector(".ql-tooltip"),
        qlToolbarArrow = qlToolbar.querySelector(".ql-tooltip-arrow");
    const editor = {

        isShow: false,
        qlToolbarWidth: 0,
        editorWidth: editorElem.clientWidth,

        showToolbar: function(x, y) {
            let bounds = editorElem.getBoundingClientRect(),
                xMid = editor.editorWidth/2 - editor.qlToolbarWidth/2,
                yOffset = bounds.top;
